...
class MembershipComponent extends BaseComponent {
    constructor() {
        super();

        this.defaultOptions = {
            selectors: {
                output: '[data-membership-output]',
                lineOutputs: {
    ...
    _getLineOutputs() {
        ...
        for (let k = 0; k < keys.length; k++) {
            ...
            if (this.$lineOutputs[key].length === 0) {
                console.log(`ERROR: membership-component.js : no line output found for data key "${key}"`);
            }
        }
    }
...
export default () => {
    return new MembershipComponent();
};
